...
class Arguments():
    ...
    Arguments().normalized_url_col (str): either the name of the column in
        the data file contains normalized URLs or the name of the column in
        the out-file that will contain normalized URLs *important*
    Arguments().infile_fieldnames (list): list of column headers in
        data file
    Arguments().enriched_fieldnames (list): list of column headers for
        out-file *important*
    """
    def __init__(self, archive, infile, urls, domains, normalized):
        ...
        with open(self.infile_path, "r", encoding="utf-8") as csvfile:
            ...
            try:
                ...
                {YELLOW}Unable to read {self.infile_path} as CSV file.{RESET}")\
                from exception
            ...
            if not normalized or test_row[self.url_col] \
                != ural_normalize_url(test_row[self.url_col]):
...
def generate_reader(infile_path):
...
@click.option("--archive", type=click.Path(exists=True), required=True, \
    help="Path to archive in which subdirectories of archived HTML will be stored.")
@click.option("--infile", type=click.Path(exists=True), required=True, \
    help="Path to the CSV file containing the URLs to be processed.")
@click.option("--urls", type=str, required=True, \
    help="Header of column containing URLs.")
@click.option("--domains", type=str, required=False, \
    help="Header of column containing domain names.")
@click.option("-n", "normalized", type=bool, is_flag=True, default=False, required=False, \
    help="Flag indicating the URLs are already normalized.")
@click.option("--outfile", "outfile_path", type=str, default="outfile.csv", required=False, \
    help="Name of or path to the file in which the enriched data will be output.")
def cli(archive, infile, urls, domains, normalized, outfile_path):
    """Parses CLI arguments, parses the CSV file, and outputs a new CSV."""
    args = Arguments(archive, infile, urls, domains, normalized)
    with open(outfile_path, "w", encoding="utf-8") as csvwriter:
        writer = csv.DictWriter(csvwriter, fieldnames=args.enriched_fieldnames)
        writer.writeheader()
